...
/**
 * Returns current setting whether to use B-frame from store if present or default (NO) otherwise.
 */
- (BOOL)currentBframeFromStore;

/**
 * Stores the provided use B-frame setting into the store.
 *
 * @param useBframe the boolean value to be stored.
 */
- (void)storeBframeSetting:(BOOL)useBframe;
