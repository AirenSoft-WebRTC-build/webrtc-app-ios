...
static NSString *const kUseBframeKey = @"rtc_use_bframe_key";
...
- (BOOL)useBframe {
  return [self.storage boolForKey:kUseBframeKey];
}

- (void)setUseBframe:(BOOL)useBframe {
  [self.storage setBool:useBframe forKey:kUseBframeKey];
  [self.storage synchronize];
}
