...
- (NSArray<RTC_OBJC_TYPE(RTCVideoCodecInfo) *> *)availableVideoCodecs {
  return [RTC_OBJC_TYPE(RTCDefaultVideoEncoderFactory) supportedCodecsWithBframe:[[self settingsStore] useBframe]];
}
...
- (BOOL)currentBframeFromStore {
  return [[self settingsStore] useBframe];
}

- (void)storeBframeSetting:(BOOL)useBframe {
  [[self settingsStore] setUseBframe:useBframe];
}
