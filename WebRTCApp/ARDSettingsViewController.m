...
- (nullable NSString *)tableView:(UITableView *)tableView
  ...
  switch (section) {
    case ARDSettingsSectionAudioSettings:
      return @"Audio";
    case ARDSettingsSectionVideoBframe:
      return @"B-frames";
    case ARDSettingsSectionVideoResolution:
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  switch (indexPath.section) {
    ...
    case ARDSettingsSectionVideoBframe:
      return [self bframeTableViewCellForTableView:tableView atIndexPath:indexPath];
...
#pragma mark - Table view delegate(B-frame)

- (UITableViewCell *)bframeTableViewCellForTableView:(UITableView *)tableView
                                         atIndexPath:(NSIndexPath *)indexPath {
  NSString *dequeueIdentifier = @"ARDSettingsBframeViewCellIdentifier";
  ...
  cell.textLabel.text = @"Enable b-frame for H.264/H.265";
  UISwitch *switchView = (UISwitch *)cell.accessoryView;
  switchView.on = [_settingsModel currentBframeFromStore];
  
  return cell;
}

- (void)bframeSwitchChanged:(UISwitch *)sender {
  [_settingsModel storeBframeSetting:sender.isOn];
...
- (UITableViewCell *)videoCodecTableViewCellForTableView:(UITableView *)tableView
  ...
  if ([codec isEqualToCodecInfoWithoutBframe:[_settingsModel currentVideoCodecSettingFromStore]]) {
    cell.accessoryType = UITableViewCellAccessoryCheckmark;
  } else {
    cell.accessoryType = UITableViewCellAccessoryNone;
  }
